class DropBoxController {
    ...
    connectFirebase(){
        var firebaseConfig = {
            apiKey: "AIzaSyDysqaOs0cRiIvd-11zJqhNlvbpkRejhAw",
            authDomain: "dropboxclone-3bd27.firebaseapp.com",
            databaseURL: "https://dropboxclone-3bd27.firebaseio.com",
            projectId: "dropboxclone-3bd27",
            storageBucket: "dropboxclone-3bd27.appspot.com",
            messagingSenderId: "537594285970",
            appId: "1:537594285970:web:7cd0ebec05f2fd64a87b4b",
            measurementId: "G-S16CVPZ9ED"
          };
    ...
    removeTask(){
        ...
        this.getSeletcion().forEach(li=>{
            let file = JSON.parse(li.dataset.files);
    ...
    initEvents(){
        ...
        this.listFilesEl.addEventListener('selectionchange', e=> {
            ...
            switch (this.getSeletcion().length){                
    ...
    uploadComplete(){
        ...
        this.this.btnSendFileEl.disable = false;
    }
    ...
    formatTimeToHuman(durantion){
        ...
        let minutos = parseInt(durantion / (1000 * 60)) % 60;
        let hours = parseInt(durantion / (1000 * 60 * 60)) % 24;

        if (hours > 0){
            return `${hours} hours, ${minutos} minutos, ${seconds} segundos`;
        }
        if (minutos > 0){
            return `${minutos} minutos, ${seconds} segundos`;
        }
        if (seconds > 0){
    ...
    getFileView(files, key){
        let li = document.createElement('li');
        li.dataset.key = key;
        li.dataset.files = JSON.stringify(files);

        li.innerHTML = `
            ${this.getFileIconView(files)}
            <div class="name text-center">${files.name}</div>
         `
